...
def cos_sim_toxic(model_id, df ,gradient_norms_compare, minus_row, minus_col):
    ...
    #del cos_all
    #del label_all
    return auprc, f1, cos_all, predicted_labels
...
if __name__ == "__main__":
    ...
    for model_id in ['meta-llama/Llama-2-7b-chat-hf']:
        ...
        auprc, f1, cos_all,label_all = cos_sim_toxic(model_id, df,gradient_norms_compare, minus_row_cos, minus_col_cos)
        df['predicted_label'] = label_all
        df['cos_all'] = cos_all
        import pdb 
        pdb.set_trace()
        df.to_csv("gradSafe_toxicchat_outputs.csv")