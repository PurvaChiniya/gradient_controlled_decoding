...
OUTPUT_DIR="${OUTPUT_DIR:-baselines/gradSafe/outputs/generations}"
...
echo "Running gradSafe controlled decoding for model=${MODEL_ID}"
...
for DATASET in "${DATASETS[@]}"; do
  ...
  CMD=(
    "${PYTHON_BIN}" baselines/gradSafe/controlled_decode.py
    --model-id "${MODEL_ID}"
    --dataset "${DATASET}"
